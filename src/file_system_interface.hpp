...
public:
    ...
    virtual int getattr(char const* path, struct stat* stbuf, struct fuse_file_info* fi) const                                                        = 0;
    virtual int readlink(char const* path, char* buf, size_t size) const                                                                              = 0;
    ...
    virtual int access(char const* path, int mask) const                                                                                              = 0;
    ...
    virtual int chmod(char const* path, mode_t mode, struct fuse_file_info* fi)                                                                       = 0;
    virtual int chown(char const* path, uid_t uid, gid_t gid, struct fuse_file_info* fi)                                                              = 0;
    ...
    virtual ssize_t read(char const* path, char* buf, size_t size, off_t offset, struct fuse_file_info*) const                                        = 0;
    virtual ssize_t write(char const* path, char const* buf, size_t size, off_t offset, struct fuse_file_info* fi)                                    = 0;
    virtual int statfs(char const* path, struct statvfs* stbuf) const                                                                                 = 0;
    virtual int release(char const* path, struct fuse_file_info* fi)                                                                                  = 0;
    virtual int fsync(char const* path, int isdatasync, struct fuse_file_info* fi)                                                                    = 0;
#ifdef HAVE_UTIMENSAT
    virtual int utimens(char const* path, const struct timespec ts[2], struct fuse_file_info* fi) = 0;
#endif // HAVE_UTIMENSAT
#ifdef HAVE_POSIX_FALLOCATE
    virtual int fallocate(char const* path, int mode, off_t offset, off_t length, struct fuse_file_info* fi) = 0;
#endif // HAVE_POSIX_FALLOCATE
    virtual off_t lseek(char const* path, off_t off, int whence, struct fuse_file_info* fi) const = 0;
};
