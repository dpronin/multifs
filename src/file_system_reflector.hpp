...
public:
    ...
    int getattr(const char* path, struct stat* stbuf, struct fuse_file_info* fi) const override;
    int readlink(const char* path, char* buf, size_t size) const override;
    ...
    ssize_t read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi) const override;
    ...
    int utimens(const char* path, const struct timespec ts[2], struct fuse_file_info* fi) override;
